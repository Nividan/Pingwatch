...
_RE_BACKUP_EXTRACT  = re.compile(r'^/api/backups/([^/]+)/extract-subnets$')
...
_RE_TLS_CA_CERTS    = re.compile(r'^/api/tls/ca-certs$')
_RE_TLS_CA_CERT_ID  = re.compile(r'^/api/tls/ca-certs/([0-9a-f]{64})$')
...
_RE_DEVICE_ROLE     = re.compile(r'^/api/device/([^/]+)/role$')
...
# Bulk Subnet Import (IPAM)
_RE_IMPORT_SUB_TPL     = re.compile(r'^/api/import/subnets/template$')
_RE_IMPORT_SUB_PREVIEW = re.compile(r'^/api/import/subnets/preview$')
_RE_IMPORT_SUB_APPLY   = re.compile(r'^/api/import/subnets/apply$')
# Auto-Discovery
_RE_AD_RUN_NOW         = re.compile(r'^/api/auto-discovery/run-now$')
_RE_AD_STATUS          = re.compile(r'^/api/auto-discovery/status$')
_RE_AD_SUPPRESS_REMOVE = re.compile(r'^/api/auto-discovery/suppressed/([^/]+)/remove$')
_RE_AD_APPROVE_FIRST   = re.compile(r'^/api/auto-discovery/subnet/(\d+)/approve-first-scan$')
