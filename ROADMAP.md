...
- **Probe types to add** — `ldap` (bind test), `postgres` / `mysql` (connection test), `radius` (authentication test), `ntp` (time-sync check), `imap` / `pop3` (mailbox auth), `mssql` (connection test) — `smtp`, `ssh`, `sftp` are shipped
