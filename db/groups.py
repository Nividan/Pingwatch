...
def db_list_groups() -> list:
    """Return [{id, name, description, member_count, ldap_dn, radius_attribute, radius_value, default_role}] ordered by name."""
    rows = db_query("main", """
        SELECT g.id, g.name, g.description, g.ldap_dn,
               g.radius_attribute, g.radius_value, g.default_role,
...
def db_create_group(name: str, description: str = "",
                    ...
                    radius_attribute: str = "", radius_value: str = "") -> int:
    ...
    try:
        with db_cursor("main") as cur:
            ...
            params = (name.strip(), description.strip(), ldap_dn.strip(),
                      radius_attribute.strip(), radius_value.strip(), default_role)
            if is_pg():
                cur.execute(
                    f"INSERT INTO user_groups (name, description, ldap_dn, "
                    f"radius_attribute, radius_value, default_role) "
...
def db_update_group(group_id: int, name: str, description: str = "",
                    ...
                    radius_value=_UG_UNSET) -> bool:
    """Update group name/description and optionally default_role / RADIUS mapping.
