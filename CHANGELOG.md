...
**Live-test refinements** (first pass on a FortiGate 3000, Nexus 5010 + FEX, and an EX2200 virtual chassis validated the walks end-to-end — composite Juniper indexes named correctly, Cisco FEX sub-device rows, 67 named Fortinet sensors — and drove three polish items): (1) **`skip_zero` item flag** — jnxOperatingTable reports 0 for inapplicable gauges (fans have "CPU 0%", PSUs "0 °C"; the MIB defines zero as *unavailable or inapplicable*), so the Juniper CPU/buffer/temp/load items now drop zero rows at discovery, collapsing ~75 preview rows to the ~20 that measure something. (2) **`type_oid` per-row units** — entity-sensor tables mix volts/°C/rpm rows under one value column; the RFC 3433 sensor-type column is now walked alongside and each row gets its real unit (Cisco "Outlet-1 47" → "47 °C", "VCore 3300 milli" → "3.3 volts"). (3) **"+ Interfaces" included by default** on Discover-with-template — the built-in Interfaces items are unioned into the same discovery (`include_interfaces` on `POST /api/snmp/discover`, checkbox default-on; uncheck for devices where ifTable is just a management NIC), so one pass covers vendor health + interfaces. (4) **"★ All templates (full scan)" picker option** — `template_id="*"` discovers against the aggregate of every template (the same union the device-scan uses, with its shorter per-OID timeout), for the "just show me everything this device answers" flow — results grouped by vendor, only responders shown. Vendor templates remain the fast targeted path.
