...
### Build 1 — MCP server for AI agents (read-only)

PingWatch now speaks **MCP (Model Context Protocol)** so AI agents (Claude and others) can connect and answer NOC questions — "what's down, why, what's the trend, what's the layout" — directly against live state. Implemented as a hand-rolled JSON-RPC server in [routes/mcp.py](routes/mcp.py) over MCP's **Streamable HTTP** transport in its **stateless JSON-only** profile: one POST to `/api/mcp` returns one JSON-RPC 2.0 response, no SSE upgrade and no session IDs (`GET /api/mcp` → 405). This deliberately avoids the official async SDK so it slots into the synchronous `ThreadingHTTPServer` with **zero new dependencies and no asyncio**, registered first in the POST dispatch ([server.py](server.py)) because a JSON-RPC body may be a list. The handshake covers `initialize` (advertises protocol `2025-06-18`, `serverInfo {name:"pingwatch", version}`, tools capability), `tools/list`, `tools/call`, and the `notifications/initialized` no-op (202, no body).

**Twelve curated, strictly read-only tools**, each calling the existing `db/`/`STATE`/`monitoring` layer in-process (so the SQLite **and** PostgreSQL paths are inherited for free) and returning slim, secret-free shapes: `list_devices`, `get_device`, `get_active_alerts`, `get_incidents` (RCA), `get_metric_history`, `get_noc_summary`, `list_sites`, `get_topology`, `search`, `get_alert_history`, `list_maintenance_windows`, `get_flaps`. Every list/history tool is **hard-capped** (default 50 / max 200 rows; history default 24h / max 30d, max 1000 points) with **cursor pagination** and an explicit `truncated`/`next_cursor` so an over-asking model paginates deliberately instead of crushing the DB or blowing its context. Tool input errors come back as MCP tool errors (`isError`), never leaking internals.

**Security model mirrors the probe jail.** A new dedicated **`mcp` token scope** ([routes/api_tokens.py](routes/api_tokens.py)) is jailed to `/api/mcp` exactly as `probe` tokens are jailed to `/api/agent/*` — enforced in both `_auth` and `_require` ([server.py](server.py)) — so an `mcp` token can reach *only* the read-only MCP endpoint and nothing else, and `read`/`full` tokens and cookie sessions are rejected *at* `/api/mcp`.

### Build 2 — MCP reporting & analytics tools (Band A)

Building a 30-day management report against the original 12-tool MCP surfaced that it was entirely event-oriented and "now"-oriented: producing basic management metrics meant pulling hundreds of raw events (90–115 KB per page) and aggregating out-of-band. This adds **three server-side analytics tools plus a state filter**, all read-only and following the existing cap/pagination/dual-backend conventions (15 tools total now):

- **`get_alert_stats`** ([db/alert_events.py](db/alert_events.py) `db_alert_stats`) — aggregated alert metrics over a window (default 24 h, max 366 d): `total`, `by_state`, `by_severity`, `suppressed_by_reason`, and **MTTR** (`avg`/`p50`/`p95` over resolved events). All counting is **SQL `GROUP BY`** — no event rows cross the wire except a capped (20 k) sample of resolved-event *durations*, with percentiles computed in Python so SQLite (no `percentile_cont`) matches PostgreSQL. `group_by` buckets by `device/sensor/severity/state/event_type/suppress_reason` in SQL, or by `site`/`group` rolled up from the top-200 devices via live `STATE` (those aren't columns on `alert_events`). This one call replaces the report's entire data-gathering phase.
- **`get_uptime`** ([db/samples.py](db/samples.py) `db_uptime_overall`/`_by_device`/`_series`) — historical **sample-based** availability (`OK samples / total samples`) over any window for the whole fleet or a `device:`/`site:`/`group:` scope, with worst-device ranking and an optional day/week time series. Reuses the raw/5m/1h rollup-table selection. The series query branches on `is_pg()` for the bucket floor (SQLite lacks `FLOOR`; PG's `CAST … AS INTEGER` rounds). Deliberately does **not** expose MTTR/MTBF/outage-count — those need outage-duration reconstruction from `flap_log` (a different definition of "availability"), planned as `get_outages`; the tool labels its method `sample_based` to keep that honest.
- **`get_audit_log`** ([db/audit.py](db/audit.py) `db_query_audit`) — the audit trail, filterable by time window, exact actor, and action prefix (e.g. `mcp_`, `alert_event_`), newest-first paginated.
- **`get_active_alerts` `state` filter** — `active` (default) `| acknowledged | suppressed | resolved | all`. The original tool only ever returned `state='active'`, so acknowledged/suppressed alerts (which carry their own states) were invisible — explaining the `noc_summary.ack` vs `get_active_alerts()=0` discrepancy without any "paused-device bug." Querying them is now possible directly.

Deferred to a later "Band B" (they need new data models first, not just a tool): notification/dispatch history, on-call schedule, and extended device inventory fields.

### Build 3 — MCP OAuth 2.1 (claude.ai web/mobile connector)

The static-token MCP server works with header-capable clients (Claude Code, `mcp-remote`), but the **claude.ai "Add custom connector" dialog authenticates via OAuth**, not a static `Authorization` header — so PingWatch now ships a minimal, spec-compliant **OAuth 2.1 Authorization Server** in front of `/api/mcp` ([routes/mcp_oauth.py](routes/mcp_oauth.py), [db/mcp_oauth.py](db/mcp_oauth.py)). It implements exactly what the MCP authorization spec needs and nothing more: RFC 9728 protected-resource metadata + RFC 8414 authorization-server metadata at `/.well-known/*` (404 when MCP is disabled — no advertising), an `/authorize` consent screen, a `/token` endpoint, and a `WWW-Authenticate` pointer on the `/api/mcp` 401 so clients can discover the flow. The async official SDK is again avoided; this is hand-rolled JSON/redirect handling on the existing synchronous server.

**Design — least privilege, reusing what exists.** Clients are **admin-pre-registered** (no open Dynamic Client Registration): an admin registers a connector under **Settings → API Tokens → MCP OAuth connectors**, naming it and its exact `redirect_uri`(s), and gets a `client_id` + `client_secret` (secret shown once, stored only as a SHA-256 hash). The `/authorize` step requires a real **admin cookie session** and shows a themed consent screen — non-admins and unauthenticated browsers get a clear "sign in as admin" page, never a token. **PKCE `S256` is mandatory**, `redirect_uri` is exact-matched (no open redirect), authorization codes are single-use with a ≤60 s TTL, and the consent POST is CSRF-protected by a session-bound nonce. Crucially, **access tokens are minted as ordinary `mcp`-scope `pw_` api_tokens** (1 h TTL) — so an OAuth-issued token validates through the *exact same* `auth_check_api_token` + jail path as a static one, with zero new validation code — while **refresh tokens rotate** (old revoked on use, 30-day lifetime) in their own table. Three dual-backend tables (`mcp_oauth_clients`, `mcp_oauth_codes`, `mcp_oauth_refresh`) seed in [db/core.py](db/core.py) + [db/pg_schema.py](db/pg_schema.py); expired codes/refresh/access rows are lazily GC'd on each `/token` call. The whole surface is gated by the same `mcp_enabled` toggle, and consent grants + client lifecycle are audited (`mcp_oauth_consent`, `mcp_oauth_client_create`, `mcp_oauth_client_delete`). Admin UI for registering/revoking connectors lives in the API Tokens tab ([frontend/forms-settings.js](frontend/forms-settings.js)). Static `mcp` tokens keep working unchanged — OAuth is purely additive.

The new scope required widening the `api_tokens.scope` `CHECK` constraint, handled as a **dual-backend, idempotent migration** that fires whenever the existing definition is missing `'mcp'`: PostgreSQL drops and re-adds the (auto-named) constraint in-place ([db/pg_schema.py](db/pg_schema.py)), while SQLite — which cannot `ALTER` a `CHECK` — does a one-time table rebuild that preserves every existing token row and the optional `probe_id` column ([db/core.py](db/core.py)), reusing the same rename→recreate→copy pattern the v1.3 `probe` scope added. The whole feature is **opt-in**: a new `mcp_enabled` setting (default **off**) makes `/api/mcp` return 503 until an admin flips the toggle in **Settings → API Tokens**, so no new network surface exists until deliberately enabled. Every `tools/call` writes one `db_log_audit` row (`mcp_tool_call`, token owner, IP, tool name, short arg summary). UI adds the `mcp` option + accent scope badge to the token generator and the enable toggle to the API Tokens tab ([frontend/forms-settings.js](frontend/forms-settings.js)). Entirely server-side — the remote probe agent is unaffected (no wire-protocol change).

### Build 4 — SNMP sensor templates & probe-aware discovery

Adding an SNMP sensor was one-OID-at-a-time: only interfaces had a bulk "Discover Interfaces" flow, so every other metric (CPU, memory, temperature, disks, PSUs, BGP peers…) had to be typed in by OID, and each vendor exposes different OIDs. PingWatch now ships **per-vendor SNMP sensor templates** — named bundles of SNMP metrics — that you pick in the Add Sensor form to **probe a device, preview only the metrics that actually responded, and bulk-add them as sensors**. A template item is either a **scalar** (one `snmpget` → one sensor) or a **table** (an OID base walked per index → one sensor per row). New dual-backend table `snmp_sensor_templates` ([db/core.py](db/core.py) + [db/pg_schema.py](db/pg_schema.py)) stores each template's items as a JSON column, with CRUD in [db/snmp_sensor_templates.py](db/snmp_sensor_templates.py) (mirrors the `report_templates` JSON pattern) exposed as admin `GET/POST/PATCH/DELETE /api/snmp/templates` ([routes/monitoring.py](routes/monitoring.py); `monitoring` was added to the `do_PATCH`/`do_DELETE` dispatch in [server.py](server.py)). A new **Settings → SNMP Templates** tab ([frontend/forms-settings.js](frontend/forms-settings.js)) manages them — create / edit / clone / delete with a per-item editor.

**Built-ins, seeded idempotently.** Thirteen templates ship pre-seeded (`source='builtin'`, keyed by `builtin_key` behind a partial-unique index so re-seeding on every start never clobbers user edits — the same guarantee as trap defs) via [snmp/seeds/snmp_templates.py](snmp/seeds/snmp_templates.py) + [snmp/seeds/loader.py](snmp/seeds/loader.py): one **Interfaces** template plus a scalar template per `SNMP_CATALOG` vendor group (Cisco, Fortinet, Juniper, Palo Alto, MikroTik, Linux/Net-SNMP, Windows, UPS, …), generated from the catalog so there is a single source of OID truth. The **Interfaces** template folds the old Discover Interfaces flow into the template system while preserving its behaviour: traffic items prefer **64-bit ifXTable counters** and derive **warn/crit from link speed** (75 % / 90 % of ifSpeed). Editing a built-in keeps the edits across re-seeds; **Reset** deletes the row so the next start re-seeds the shipped default.

**Discovery engine.** The interface walker's inner subtree walk was extracted into a reusable `snmpwalk_oid()` (interface discovery is byte-for-byte unchanged), and a new `snmp_discover_template()` ([monitoring/probes.py](monitoring/probes.py)) probes a template against a device: scalars via concurrent `snmpget`, tables via one walk of each distinct value/name/speed OID (resolving each row's name from ifName → ifDescr → index), returning **only responders** as candidate rows with a server-built `suggested_name` ("eth0 In Traffic" for tables, "CPU Load" for scalars). `POST /api/snmp/discover` backs the Add Sensor picker + a checkable, filterable preview ([frontend/forms-sensor.js](frontend/forms-sensor.js)); bulk-add loops `POST /api/device/<did>/sensor` with threshold precedence **item warn/crit → interface speed-auto → form default**. The v3-credential resolution + `effective_probe` routing shared by interfaces / discover / scan was factored into `_snmp_conn_params` / `_snmp_route` helpers.

**Probe-routed, like interface discovery.** Discovery on a probe-bound device runs **on the probe** (central usually can't reach a branch device) via a new `snmp_discover` agent task ([routes/agent.py](routes/agent.py) `run_remote_snmp_discover` + [agent/agent.py](agent/agent.py) `_run_snmp_discover`), reusing the synchronous long-poll waiter (`_SYNC_WAITER_TASKS`) the v1.4 `snmp_interfaces` task added — it never falls back to a local walk and surfaces a clear 409/503 when the probe is unenrolled/offline.

**Scan device → build template.** A **🔍 Scan Device** action in the Settings tab probes a device against **every known OID** (all templates ∪ the catalog, deduped by OID via `_snmp_all_known_items`), groups the responders by vendor with their live values, and saves the selection as a **new template** (admin `POST /api/snmp/scan`, short per-OID timeout, table rows collapsed back to base items) — an authoring aid that turns a real device into a reusable template. No sensors are created by the scan.

**Agent parity / fleet impact.** `monitoring/probes.py` and `agent/agent.py` changed, so the agent **build id changes → the managed fleet needs an agent update** before probe-side discovery/scan works there; central degrades gracefully with an "older agent build" message until then, and interface discovery keeps working via the legacy path. Wire protocol unchanged (still `1`) — the new task type is additive.

### Build 5 — SNMP discovery correctness: absent-OID handling + catalog OID fixes

Testing the Build 4 templates against live FortiGate and Cisco gear surfaced a correctness bug and a batch of hand-curated catalog errors. **`probe_snmp` was treating an absent OID as a successful read**: net-snmp returns exit status 0 with an *exception varbind* (`noSuchObject` / `noSuchInstance` / `endOfMibView`) for an OID the agent doesn't implement, so a sensor pointed at a dead OID read **"up" with the literal "No Such Object available…" text as its value**, and template discovery listed every dead OID as a responder. Fixed at both SNMP entry points in [monitoring/probes.py](monitoring/probes.py): the scalar `probe_snmp` now returns `ok:False` for a type-less exception varbind, and the shared walk parser `_snmpwalk_indexed` skips exception lines (so an absent *table* base can't emit a phantom `{index: "No Such Object…"}` row). Both live in the agent-parity `probes.py`, so the fix ships to remote probes and **bumps the agent build id**.

**Catalog OID corrections** ([snmp/catalog.py](snmp/catalog.py)) — the live values proved several hand-entered OIDs were mislabeled. **Fortinet:** Serial Number pointed at `fgSysMgmtVdom` (`.101.4.1.2.0`, the VDOM index) → corrected to `fnSysSerial` `1.3.6.1.4.1.12356.100.1.1.1.0` (FORTINET-CORE-MIB, `.100` not `.101`); Memory Capacity relabeled **KB** (was MB — `fgSysMemCapacity` reports KB, so it read ~94 TB); the `.4.1.8–.11` block was shifted and is really the `fgSysSesCount`/`SesRate1`/`Ses6Count`/`Ses6Rate1` family → relabeled Active Sessions / Session Setup Rate / IPv6 Sessions / IPv6 Session Rate, with uptime moved to standard `sysUpTime` `1.3.6.1.2.1.1.3.0`; the hardcoded per-interface `fgIntf` rows were dropped in favour of the Interfaces template. **Cisco:** the `cpmCPUTotalTable` columns were wrong — "CPU 1-min" pointed at `.3` (`cpmCPUTotal5sec`, deprecated → absent on modern IOS-XE) and "CPU 5-min" at `.7` (`cpmCPUTotal1minRev`, actually the 1-minute value) → corrected to the current Rev columns `.7` (1-min) / `.8` (5-min), and the IOS-legacy fallback from `busyPer` `.56` to `avgBusy5` `.58`. **Juniper:** the `jnxOperatingTable` Routing-Engine columns were scrambled (CPU sat on `.11`/buffer, Temperature on `.8`/CPU, DRAM on `.7`/temperature, Uptime on `.2`/an index — so the "38 MB DRAM" reading was really 38 °C) → corrected against JUNIPER-MIB to Temp `.7` / CPU `.8` / Buffer `.11` / UpTime `.13` (1/100 s) / installed-memory `.15` (MB — deliberately not the bytes-valued `.10`, which the `_BYTE_UNITS` classifier would treat as a counter/rate). The hardcoded per-interface rows were also dropped from the Fortinet and Juniper groups in favour of the Interfaces template.

**Upgrade note:** built-in templates seed idempotently on `builtin_key`, so an already-seeded install keeps its old template items even after this ships. To pick up the corrected OIDs on an existing install: deploy, then **Reset** the affected built-ins (Fortinet, Cisco, Juniper) in Settings → SNMP Templates and **restart once more** — Reset deletes the row (frontend `_snmpTplDelete`) and the corrected template re-seeds at the next startup (`load_all_seeds`). Fresh installs get them automatically. *(Superseded by Build 6's versioned seed refresh — unedited built-ins now update automatically.)*

### Build 6 — Professional SNMP template library: 21 MIB-verified vendor templates + walked tables, computed %, value scaling

Live testing kept proving the catalog-generated templates wrong in a systemic way: OIDs typed from memory and **table metrics pinned to a fixed index** (`.1`) that silently miss whenever a device's real index differs (Cisco N5K interfaces, Juniper RE at `9.1.0.0`). This build replaces them with a professionally-authored library — **21 built-in templates, 204 items, every OID verified against the vendor MIB** (oidref / mibbrowser / Observium / LibreNMS / vendor docs, cited per module) — plus the four engine capabilities professional templates require.

**Engine.** (1) **Composite-index walks** — `_snmpwalk_indexed` now keys rows by the full index suffix after the base ("9.1.0.0" for `jnxOperatingTable`, host.cpu.slot for F5 per-core CPU, MAC-indexed Meraki/Aruba tables) instead of only the last component, which collapsed composite tables to one row; `snmpwalk_interfaces` coerces back to int keys so interface discovery is byte-for-byte unchanged ([monitoring/probes.py](Pingwatch/monitoring/probes.py)). (2) **Value scaling** — template items and sensors carry a `scale` divisor (MikroTik deci-°C ÷10, KB→MB ÷1024, APC runtime TimeTicks ÷6000→minutes); new `sensors.snmp_scale` column (both backends), applied **centrally in `_process_result`** so local and remote results scale identically — no agent-side scale logic; counter types exempt (the agent computes rates from raw values). Table items may instead carry `scale_oid`+`precision_oid` — the **RFC 3433 entity-sensor columns** are walked alongside the value and each row gets an auto-computed divisor (`10^(precision−scaleExp)`, honoring the RFC's exa(14)=10¹⁸/peta(15)=10¹⁵ quirk), so Cisco/PAN/Aruba-CX sensors read 3300 milli-volts as 3.3 V without per-model hardcoding. (3) **Computed-% sensors** — most vendors expose memory as used+free/total counters, not a percent; items (and sensors, via new `snmp_oid2`+`snmp_pct_mode` columns) combine two OIDs per mode (`used_total`/`used_free`/`free_total`), as scalars or **per-row on walked tables** (hrStorage → one "Used %" sensor per filesystem, allocation units cancel). `probe_snmp_percent` runs on central and the agent ([agent/agent.py](Pingwatch/agent/agent.py) dispatch + config push). (4) **Enum primary = first-listed code** — the healthy state of an enum legend used to be hardcoded to "1", which would fire *inverted* state events for the many professional enums where 1 is a fault (F5 `1=good 0=bad`, Fortinet `0=ok 1=alarm`, Dell `3=ok`, HPE `2=ok`, Cisco FRU `2=on`); now the first pair listed wins, in the Python engine and the JS mirror (which parses the raw string — JS objects reorder integer keys), fully back-compat since every existing legend lists 1 first.

**Templates** ([snmp/seeds/templates/](Pingwatch/snmp/seeds/templates/), one cited module per vendor family): Generic Host (Host-Resources + UCD — per-core CPU, per-filesystem storage %, load avg, swap), Cisco IOS/IOS-XE/NX-OS (cpmCPUTotal *Rev* columns, enhanced+legacy mempool %, ENVMON, entity sensors auto-scaled, FRU fan/PSU, ENTITY identity), Fortinet FortiGate (incl. conserve-mode low-memory, per-tunnel IPsec status, HA member stats), Juniper JunOS (jnxOperatingTable walked+named — CPU/buffer/temp/state incl. fan/PSU rows, FRU state, alarms), Palo Alto PAN-OS (native session metrics + host-resources CPU/mem + entity sensors), MikroTik RouterOS (health scalars deci-scaled, hrStorage %), HP ProCurve, ArubaOS-CX, F5 BIG-IP (per-core CPU, TMM vs host memory %, inverted fan/PSU enums), UPS RFC-1628 + APC PowerNet, VMware ESXi (SNMP-only scope stated), Cisco Meraki cloud (inventory/reachability — the cloud MIB has no health objects), Aruba Mobility Controller + Instant, Ubiquiti UniFi/airMAX/EdgeOS, Dell iDRAC (tenths auto-divided, LocationName-named probes), HPE iLO/ProLiant (corrected `cpqHeTemperature*` columns). `snmp/catalog.py` is retired as the template source (remains the manual OID picker + scan aggregate); `_catalog_templates()` generation is gone.

**Versioned seed refresh** ([db/snmp_sensor_templates.py](Pingwatch/db/snmp_sensor_templates.py)) — the old INSERT-OR-IGNORE seeding meant shipped corrections never reached existing installs. Built-ins now carry a `builtin_version` content hash + `user_modified` flag (both backends, migrated): on every start, new keys insert, **un-edited built-ins whose shipped content changed update in place**, user-edited ones are never touched, and un-edited built-ins whose key is no longer shipped are pruned (cleanly retiring the old `builtin:catalog:*` rows). Any PATCH marks the row user-modified; **Reset now restores the shipped default immediately** (the DELETE route re-seeds that key on the spot — no restart).

**UI.** Template editor gains the Percent A/B kind, partner-OID + mode row, ÷scale field, and table extras (fallback name OID, RFC 3433 scale/precision/type OIDs, skip-zero). Discovery preview + device-scan now show **live values on table rows too** (scaled/computed server-side); bulk-add passes `snmp_scale`/`snmp_oid2`/`snmp_pct_mode` onto created sensors; scan→save-as-template collapses composite-index rows correctly and preserves percent pairs ([frontend/forms-settings.js](Pingwatch/frontend/forms-settings.js), [forms-sensor.js](Pingwatch/frontend/forms-sensor.js)).

**Live-test refinements** (first pass on a FortiGate 3000, Nexus 5010 + FEX, and an EX2200 virtual chassis validated the walks end-to-end — composite Juniper indexes named correctly, Cisco FEX sub-device rows, 67 named Fortinet sensors — and drove three polish items): (1) **`skip_zero` item flag** — jnxOperatingTable reports 0 for inapplicable gauges (fans have "CPU 0%", PSUs "0 °C"; the MIB defines zero as *unavailable or inapplicable*), so the Juniper CPU/buffer/temp/load items now drop zero rows at discovery, collapsing ~75 preview rows to the ~20 that measure something. (2) **`type_oid` per-row units** — entity-sensor tables mix volts/°C/rpm rows under one value column; the RFC 3433 sensor-type column is now walked alongside and each row gets its real unit (Cisco "Outlet-1 47" → "47 °C", "VCore 3300 milli" → "3.3 volts"). (3) **One discovery control** — the SNMP block is now just a template picker + **⊕ Discover**: the legacy "Discover Interfaces" button is retired from the UI (fully superseded — the Interfaces template does the same walk with speed-thresholds + 64-bit counters; `/api/snmp/interfaces` and its JS stay one release for compatibility), and interfaces are **always unioned into template discovery** server-side (`include_interfaces` defaults on; API callers can pass false). (4) **"★ All templates (full scan)"** — the picker's first, pre-selected option: `template_id="*"` discovers against the aggregate of every template (the same union the device-scan uses, with its shorter per-OID timeout), so the default flow is "open form → Discover → everything the device answers, grouped by vendor". Picking a vendor template remains the faster targeted path.

**Verification.** Full suite against the real functions on throwaway SQLite: fresh seed = 21 templates / stale-catalog prune / idempotent re-seed (no id churn) / content-hash refresh / user-edit preservation; composite-index walk with per-row naming; milli+precision auto-scale (3300→3.3 V, 305→30.5 °C); per-row and scalar computed-% (750/1000→75); static scale (415→41.5); dead-OID filtering; remote candidate sanitize (composite index kept, junk rejected); **interface discovery regression — byte-for-byte identical output**. All 204 authored items round-trip the route validator with no field loss.

**Agent parity / fleet impact.** `monitoring/probes.py` (walk, percent probe, discovery transforms) and `agent/agent.py` (percent dispatch) both ship to the agent → **build id changes → managed fleet needs an agent update** for probe-side percent sensors and enriched discovery; central degrades gracefully on older builds. Wire protocol unchanged. The `snmp_scale` divisor is deliberately applied central-side only, so scaled sensors work on old agents too.

### Build 7 — SNMP template export / import (JSON files)

The template editor now carries a lot of per-item options (scale, percent partner OIDs, RFC 3433 columns, flags), which makes bulk authoring in the GUI tedious — so templates are now portable JSON files. Each row in **Settings → SNMP Templates** gains an **Export** button that downloads the template as a pretty-printed `snmp-template-<name>.json` containing only the portable fields (`name`/`vendor`/`description`/`items` plus a `pingwatch_snmp_template: 1` format marker — no DB ids or builtin markers), and the tab footer gains **⤒ Import**, which accepts a single template object, a bare array, or a `{templates:[…]}` bundle and creates each entry as a CUSTOM template ([frontend/forms-settings.js](frontend/forms-settings.js)). This enables the hand-edit loop (export → edit items in a text editor → import), sharing templates between installs (including managed fleet servers, where templates are DB config, not code), and text-editor authoring of large templates. **Entirely frontend** — import posts through the existing admin-only `POST /api/snmp/templates`, so every item is re-validated server-side by `_snmp_tpl_clean_items` exactly like a GUI save (junk fields dropped, bad items rejected); name collisions with an existing template get an ` (imported)` suffix instead of silently duplicating or overwriting; per-import audit rows come free from the existing route. Exporting a built-in and re-importing it yields an editable custom copy while the shipped built-in stays versioned-refreshable. No backend, DB, or agent changes.

### Build 8 — Pre-release audit: correctness, security & reliability fixes

A pre-release review turned up a batch of correctness, security, and reliability issues; this build fixes them. Grouped by area:

**Alerting lifecycle (stop alerts from lying).**
- **Reset SNMP built-in no longer wipes the others.** "Reset to default" on one built-in template passed a single-element list to `db_seed_snmp_templates`, whose prune pass then deleted every *other* un-edited built-in until the next restart. Seeding takes a `prune` flag now; single-key reset passes `prune=False` ([db/snmp_sensor_templates.py](db/snmp_sensor_templates.py), [routes/monitoring.py](routes/monitoring.py)).
- **Pausing a device/sensor closes its incident.** `stop_device`/`stop_sensor` resolved flaps only, so an acknowledged alert on a paused device lingered `active` forever and its stale ACK then silenced the *next* incident on resume. They now resolve alert events + clear stage state too (skipped on shutdown / probe-reassignment swaps) ([core/state.py](core/state.py)).
- **Ack has a state guard.** Acking a *resolved* event flipped it back to `acknowledged`, re-surfacing it and arming the ack-gate against the sensor's next incident. `db_ack_event` now only transitions open events; the route returns 409 otherwise ([db/alert_events.py](db/alert_events.py), [routes/alert_events.py](routes/alert_events.py)).
- **Recovery isn't lost inside a maintenance window.** The maintenance (and license/cert) recovery path returned before dispatching *and* before auto-resolving, so an outage that recovered inside a window left operators with a DOWN and no "recovered", and stranded the event `active`. Recovery is now exempt from maintenance suppression (matching the RCA gate); the `had_prior`/`fire_count` guard means no spurious-recovery noise ([monitoring/alert_profile_engine.py](monitoring/alert_profile_engine.py), [monitoring/license_checker.py](monitoring/license_checker.py), [monitoring/cert_alert_checker.py](monitoring/cert_alert_checker.py)).
- **Disabling/deleting the only matching profile mid-incident no longer strands the event.** The engine's no-profile / all-disabled early returns now resolve a lingering open event once the sensor is healthy ([monitoring/alert_profile_engine.py](monitoring/alert_profile_engine.py)).
- **Suppressed events are pruned.** `db_clean_alert_events` only deleted `resolved` rows; `suppressed` rows (terminal, `resolved_at=0`) accumulated forever and inflated `get_alert_stats`. They're now pruned by `triggered_at` on the same retention ([db/alert_events.py](db/alert_events.py)).

**Distributed probes — clock skew (a slow branch clock silently stopped alerting).** A remote agent stamps samples with its own clock; the server only clamped *future* timestamps, so a branch clock running >~2 min slow had every live result filed as stale backfill — samples stored, but **no threshold/flap/alert evaluation** for that branch, invisibly. `_ingest_results` now corrects each sample by the measured `clock_skew_s` (agent-minus-server offset), which realigns a mis-set clock while leaving genuine offline-spool backfill (skew≈0) untouched; a future-ts backstop remains. The watchdog warns + audits when a probe's offset exceeds half the 120 s stale floor, so broken NTP on a branch is visible instead of silent ([routes/agent.py](routes/agent.py), [monitoring/probe_watchdog.py](monitoring/probe_watchdog.py)). Server-only — the agent already reports `agent_now`; no fleet update.

**Security.**
- **Bundle-import zip-slip closed.** `restore_secrets_from_zip` wrote bundled TLS-cert entries using the path from the zip with no containment check, so a crafted `secrets/certs/../../…` member could plant a file outside `CERTS_DIR` (new `authorized_keys` / `.pth` / cron → RCE). Now guarded with the same abspath-containment check as the image extractor ([core/backup_bundle.py](core/backup_bundle.py)).
- **LDAP/AD TLS verification (opt-in, staged).** LDAPS/StartTLS used `CERT_OPTIONAL` (no cert/CA check), so the AD bind — carrying the service-account and every user password — was MITM-able. A new `ldap_tls_verify` setting (default **off** so existing logins don't break on deploy) enables `CERT_REQUIRED`, trusting the OS store plus CAs uploaded under **Settings → Platform → Certificates → Trusted CA Certificates** (the internal-CA case; made truly additive in Build 12); UI toggle + test-connection override included ([core/ldap_auth.py](core/ldap_auth.py), [routes/ldap.py](routes/ldap.py), [frontend/forms-settings.js](frontend/forms-settings.js)).
- **TOTP secret encrypted at rest.** The 2FA seed was stored in cleartext (every other auth secret is Fernet-encrypted), so a DB leak / pg_dump exposed every seed. It's now Fernet-encrypted, with a transparent one-time migration of legacy plaintext rows on first read ([db/users.py](db/users.py)).
- **SSO + TOTP now completes.** A TOTP-enrolled SAML/OIDC user was redirected to `/?sso_totp=…` that **nothing consumed** — they could never finish logging in. Added the shared pending-2FA store, the `POST /api/login/sso-totp` consumer (verifies code/recovery, rate-limited, audited), and the frontend prompt ([core/sso_common.py](core/sso_common.py), [routes/auth.py](routes/auth.py), [routes/oidc.py](routes/oidc.py), [routes/saml.py](routes/saml.py), [frontend/app.js](frontend/app.js)).

**Upgrade & backup safety.**
- **PostgreSQL restore is non-destructive.** Rollback and bundle-import did `DROP SCHEMA … CASCADE` then a separate, non-transactional load — a failed/corrupt load left an empty schema (bricked server / wiped data). Both now use a blue-green schema swap: rename the live schema aside, load into a fresh one, revert on failure, drop the backup on success ([bootstrap.py](bootstrap.py), [routes/export.py](routes/export.py)).
- **Probation rollback target preserved.** `_begin_probation` recomputed `previous` from the live pointer, so a crash between the pointer flip and the state save could set the rollback target to the bad release itself. It now trusts the target `stage_image` recorded ([bootstrap.py](bootstrap.py)).
- **Crash-loop rollback no longer discards data.** A committed release that ran (and migrated the DB) for weeks before crash-looping had its *pre-upgrade* snapshot silently restored, throwing away everything since. Crash-loop rollback is now code-only (DB left intact, logged for review); the probation-timeout path (fresh upgrade) still restores ([bootstrap.py](bootstrap.py)).
- **Remote backup retention prunes bundles.** The SFTP/SMB prune matched only the legacy `pingwatch-main-`/`pingwatch-logs-` prefixes, so v1.5 `pingwatch-bundle-*` artifacts grew unbounded off-box. Added the bundle prefix ([backup/remote_upload.py](backup/remote_upload.py)).

**Maintenance-window editor.** Rendering a stored window into the `datetime-local` picker used UTC while save re-parsed as local, drifting one-time windows by the browser's UTC offset on every edit (alerts suppressed at the wrong time → real alerts missed). The picker now renders local wall-clock ([frontend/alerting.js](frontend/alerting.js)).
...
### Build 10 — SNMP discovery: hide unused/empty metrics in the preview

Discovering a device against a vendor template unions the built-in Interfaces template by default (so one discovery covers vendor health **and** every interface). On a chassis with many ports that floods the pick-list — a 56-interface FortiGate returns ~529 candidate rows, ~450 of them IF-MIB metrics for ports that are administratively/operationally down. The discovery preview now has a **Hide unused/empty** toggle (default on, shown only when there's something to hide) that collapses blank-valued rows and every metric of an operationally-down interface, keyed off each interface's `ifOperStatus` (ifIndex → status map built from the discovered `1.3.6.1.2.1.2.2.1.8.*` rows). It is deliberately **not** a blunt "hide any 0": health scalars reading 0 (CPU, sessions, IPsec-tunnels-up), healthy hardware-sensor rows (`0=ok`), and error/discard counters on *up* interfaces are legitimately zero and stay visible — only dead interfaces are dropped. The free-text filter and the toggle compose, and Select-all/Add act on visible rows only, so one pass adds sensors for the in-use interfaces + vendor metrics without wading through the unused ports ([frontend/forms-sensor.js](frontend/forms-sensor.js)). Frontend-only; no API, DB, or agent change.

### Build 11 — Executor auto-resize: debounce the pool rebuild, drain the old pool

Auto-scaling the probe thread pool (`sensors // 4`, floor 64, cap 512) fired on every single sensor change via `_maybe_resize_executor`, and each 4-sensor step **replaced** the whole `ThreadPoolExecutor`. So a bulk add — e.g. hundreds of interface sensors from SNMP discovery — rebuilt the pool once per 4 sensors, each rebuild abandoning the old pool for GC and emitting an `INFO` line (a ~400-sensor add logged ~100 of them, contradicting the "INFO = significant events" convention). The resize is now **debounced** behind a 3 s reset-on-each-call timer: a burst of adds / deletes / imports collapses into a **single** resize once it settles, so the pool is rebuilt once and the now-meaningful INFO line is logged once. The swap also `shutdown(wait=False)`s the old pool so its idle threads exit promptly instead of lingering until GC — in-flight probes finish on it first, and the scheduler's already-guarded submit ([core/state.py](core/state.py)) re-queues the rare "probe submitted to the pool mid-swap" case (+5 s) rather than dropping it. The scaling formula, floor, and cap are unchanged; the count is snapshotted so a concurrent add can't race the tally ([routes/devices.py](routes/devices.py)). Server-only; no API, DB, or agent change.

### Build 12 — Device detail window: lazy sparklines + chunked render for sensor-dense devices

Opening a device's detail window rendered **every** sensor synchronously — a DOM tile + `<canvas>` each in one `forEach` ([devices.js](frontend/devices.js) `openDevWin`), then `setupChartsByDid` looped every sensor **again** to `getContext('2d')` + draw a sparkline. For a handful of sensors that's instant, but SNMP interface discovery makes 200–500-sensor devices easy, and there the two O(N) synchronous passes (plus a per-tile forced reflow) froze the window on open. Three changes, no UX compromise — all sensors stay in one filterable, scrollable view:

- **Lazy sparklines.** While a device window is open, tiles register with an `IntersectionObserver` scoped to the window's scroll area (`.dw-body`) and draw their canvas only once scrolled into view (one-shot). ~15–20 visible tiles draw instead of all 250; the rest draw on scroll. Live updates already no-op when a chart doesn't exist yet, and `S.sensors[key].history` stays fresh via SSE, so a deferred draw isn't stale ([frontend/sensors.js](frontend/sensors.js), [frontend/devices.js](frontend/devices.js)).
- **Chunked render.** Tiles build ~40 per animation frame instead of one blocking loop, so the window opens immediately (skeletons → tiles stream in) and stays responsive.
- **Payload trim.** The device GET shipped all 80 history points per sensor, but the tile sparkline only needs a short preview — capped to the last 20 (`_SPARK_POINTS`); the detail chart still fetches the full series on demand via `/api/sensors/{did}/{sid}/history`. Cuts the payload ~4× on sensor-dense devices ([routes/devices.py](routes/devices.py)).

VMware VM-row sparklines keep their existing eager draw but still gain the chunked render; browsers without `IntersectionObserver` fall back to an eager (chunked) draw. The only server change is the payload trim; no agent change.

### Build 12 — LDAP TLS verification: uploaded CAs now add to the OS trust store (not replace it)

The Build 8 LDAP verify toggle promised "OS trust store plus any uploaded CA", but delivered something narrower: with **any** CA uploaded under Trusted CA Certificates, ldap3 received only that PEM as `ca_certs_data` — and both ldap3 2.9.1 and Python's `ssl.create_default_context()` load explicit CA data **instead of** the OS defaults, not on top of them. So uploading an internal CA for HTTPS sensors silently narrowed LDAP trust to that CA alone: enable verify against an AD presenting a public-CA (or different-internal-CA) certificate and every domain login fails, even though the OS store would have validated it — the opposite of the Certificates tab's stated "System CAs remain trusted; uploaded CAs are added on top" convention (which the sensors, via `apply_trusted_cas`, honor correctly).

Fixed by exporting the OS default store to PEM once per process — new `get_system_ca_pem()` in [core/ssl_trust.py](core/ssl_trust.py) (`create_default_context().get_ca_certs(binary_form=True)` → `DER_cert_to_PEM_cert`, cached; cross-platform over the Windows cert store and Linux bundles) — and appending it to the uploaded blob before handing it to ldap3's `Tls` ([core/ldap_auth.py](core/ldap_auth.py)). On rare capath-only distros the export can come back empty, in which case behavior degrades to the previous uploaded-CAs-only — never worse. With no uploaded CAs the path is unchanged (plain `CERT_REQUIRED` → OS store). Verified during the same review: ldap3 performs post-handshake hostname validation under `CERT_REQUIRED`, so the toggle's MITM protection is real. Also fixed the toggle's stale help text — the CA upload lives under **Platform → Certificates → Trusted CA Certificates**, and the reference is now a clickable link that jumps to that tab ([frontend/forms-settings.js](frontend/forms-settings.js)). Server + frontend only; LDAP is central-side, the agent's `ssl_trust` shim is separate — no wire-protocol or agent change.
...
### Configure per-type sensor defaults before any sensors exist

The **Settings → Sensors** per-type defaults table used to build its rows only from sensor types that *already had sensors*, so on a fresh server (no devices yet) the section was empty — and the per-type Interval / Timeout / Warn / Crit and type-specific fields couldn't be set up until after sensors existed, even though those defaults are exactly what new sensors inherit. The table now renders a row for **every supported type from the catalogue**, unioned with any live types not in it (e.g. `vmware`), so an operator can configure defaults *before* adding the first sensor ([frontend/forms-settings.js](frontend/forms-settings.js) `loadSensorsDefaultsTab` builds from `_SDR_META` ∪ live counts). The count column shows `0` and the row's **⤓** "apply to existing" button is disabled with a tooltip when a type has no sensors yet ([frontend/style.css](frontend/style.css) `.sdr-apply-btn:disabled`); **Save Sensor Defaults** persists the per-type blob (`snr_type_defaults`) as before. Display-only change — no new settings, no schema change.

### Quieter logs on TLS client disconnects

`QuietServer.handle_error` ([server.py](server.py)) already swallowed browser-disconnect tracebacks for `ConnectionAbortedError` / `ConnectionResetError` / `BrokenPipeError` / `SSLEOFError`, but a client dropping a TLS connection mid-flush can surface as a plain `ssl.SSLError: [SYS] unknown error (_ssl.c:NNNN)` (an `SSL_ERROR_SYSCALL`/EOF) — most often on the long-lived `/events` SSE stream. `"SSLError"` is **not** a substring of `"SSLEOFError"`, so the old list missed it and a full stack trace hit the log on every such disconnect. Added the base `SSLError` (and the clean-close `SSLZeroReturnError`) to the ignore list. These only ever reach `handle_error` per-connection (cert/key load errors surface at startup on a different path), so nothing actionable is hidden — behaviour is identical, just without the noise.
