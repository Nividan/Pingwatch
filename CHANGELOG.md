...
- **Startup grace — no more restart blips.** After a restart the first probe cycle used to spray DOWN events that auto-resolved one cycle later (worst with vCenter: the pyvmomi session cache is in-memory, so every vmware sensor's first probe paid a 5-20s SmartConnect login and timed out). Three-part fix: (1) a **startup grace window** (`startup_grace_s` setting, default 60s, 0=off) parks new down/threshold events — probes run, samples record, tiles go red live, but the event row + alerts only emit if the sensor is *still* failing when the window closes, stamped with the **true transition time**; blips that recover inside the window vanish without a trace, and recoveries/auto-resolves always process immediately ([core/state.py](core/state.py) `begin_startup_grace`/`_flush_grace`). (2) The probe watchdog holds its first `probe_offline` verdicts until agents have had time to reconnect — right after boot every probe's persisted `last_seen` is stale by definition. (3) **vCenter session pre-warm**: server and agent both background-login to each distinct vCenter at startup (`prewarm_session` in [vmware/client.py](vmware/client.py)), and the agent gives vmware sensors an 8s first-probe head start — removing the failures at the source.
