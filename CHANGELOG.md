...
- **LDAP/AD TLS verification (opt-in, staged).** LDAPS/StartTLS used `CERT_OPTIONAL` (no cert/CA check), so the AD bind — carrying the service-account and every user password — was MITM-able. A new `ldap_tls_verify` setting (default **off** so existing logins don't break on deploy) enables `CERT_REQUIRED`, trusting the OS store plus CAs uploaded under **Settings → TLS → CA certificates** (the internal-CA case); UI toggle + test-connection override included ([core/ldap_auth.py](core/ldap_auth.py), [routes/ldap.py](routes/ldap.py), [frontend/forms-settings.js](frontend/forms-settings.js)).
