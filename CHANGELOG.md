...
Follow-up fixes for user-reported regressions after the audit-hardening restart.

- **`verify_ssl` silently flipped to true on existing sensors** — the per-VM Edit Metrics modal (`frontend/sensors.js::openEditVmMetrics`) derived its reference `refVssl` from `ref.verify_ssl !== 0`. Backend sends JSON boolean `false`, and `false !== 0` is `true` under strict inequality (different types), so every new metric added through the modal was POSTed with `verify_ssl: true` regardless of the source sensor. Same type bug in the Add/Edit Sensor form's checkbox render (`frontend/forms-sensor.js` http / http_keyword / vmware templates) — `s?.verify_ssl === false ? '' : 'checked'` treated `0` as checked. Replaced both with truthy coercion (`!!ref.verify_ssl` and `s && !s.verify_ssl ? '' : 'checked'`)
- **Edit Metrics showed all checkboxes unchecked for mixed-metric hosts** — `_ensureVmGrp` decided host-vs-VM from the first-rendered sensor's metric prefix, and `openEditVmMetrics` filtered the catalogue to the caller's `isHost`. A host with a stray `on` (VM) sensor alongside 18 `host_*` sensors could mislabel the whole group as VM, filter the catalogue to VM metrics, and show nothing ticked. Refactored to use the VMware MoID prefix as the authoritative entity type (`vm-*` / `host-*` / `datastore-*`) via two new helpers: `_vmEntityType(vmid)` and `_metricCategory(metricKey)`. `_vmGrpIsHost` now thin-wraps `_vmEntityType`; the Edit modal filter keeps every metric already monitored plus the catalogue entries matching the entity type. That also blocks offering datastore metrics on a VM MoID — a probe would fail with "Datastore vm-X not found"
- **Edit Metrics errored "catalogue not loaded yet" on cold sessions** — the three VMware catalogues (`_vmwareMetrics`, `_vmwareHostMetrics`, `_vmwareDatastoreMetrics`) were only populated by the Add Sensor flow in `forms-sensor.js`. Users opening Edit Metrics before ever touching Add Sensor hit an error toast. New `_ensureVmwareCatalogue()` helper fetches `/api/vmware/metrics` + `host-metrics` + `datastore-metrics` in parallel, only for arrays still `null`. `openEditVmMetrics` awaits it before proceeding
- **Edit Metrics demanded a password when the device already had one** — the modal required a typed password for every add. The backend (`routes/devices.py::_vm_pw_from_device`) already falls back to `dev.vmware_password_default` when the POST omits `vmware_password`. Frontend now reads `S.devices[did].has_vmware_password_default`, labels the password field "*(optional — device default will be used if blank)*", and `_vmEditSave` only rejects blank passwords when no device default exists. Typing a password still overrides
- **"VMware sensor created without SSL verification" warning fired for edits** — `routes/devices.py` emitted this once per host per process on first `POST /api/device/:did/sensor` with `verify_ssl=false`. Accurate, but misleading when the Edit Metrics modal's "add a metric" flow is also a POST. Reworded to *"VMware: sensor on %s has Verify SSL disabled — enable for production use"* — neutral for both flows
- **Log-entries badge stayed invisible after any server restart** — `core/logger.py::_badge_total` is in-memory and resets to 0 each start, but the browser's `logBadgeSeen` localStorage key persists. After one badge click, `seen` was typically ahead of `total` on the next startup, so `max(0, total - seen)` stayed 0 forever no matter how many new WARNINGs came in. `_logBadgeInit` now detects the restart (`seen > total`) and resets the watermark. Separately, switching to the new top-level Logs tab now marks the badge seen (before, only clicking the badge itself did) — matches pre-split behaviour where visiting the Logs panel in Settings effectively cleared it
- **Files** — [frontend/sensors.js](Pingwatch/frontend/sensors.js) (`_vmEntityType` + `_metricCategory`, `_vmGrpIsHost` rewrite, `openEditVmMetrics` catalogue ensure + entity-aware filter + device-default password fallback, `refVssl` truthy coercion, `_vmEditSave` password-optional branch); [frontend/forms-sensor.js](Pingwatch/frontend/forms-sensor.js) (`_ensureVmwareCatalogue` helper, checkbox render `!s.verify_ssl` on as-vssl / as-kwssl / as-vmssl); [frontend/app.js](Pingwatch/frontend/app.js) (`_logBadgeInit` restart-detect, `switchMainTab('logs')` marks badge seen); [routes/devices.py](Pingwatch/routes/devices.py) (reworded VMware SSL warning)

---

## 🛑 Shutdown-ordering race — trailing "pool is closed" and "cursor already closed"

Reported after the audit-hardening restart: clean `systemctl stop` logged `InterfaceError: cursor already closed` immediately after `PostgreSQL pool closed.`, plus a burst of `DB err log error: PostgreSQL pool is closed` / `db_record_stage_fire error` / `db_log_event error` ending with `alert_batcher: drained 18 pending alert(s) on shutdown`. Two distinct races behind the same symptom.

- **Probe workers outran `pg_close_pool()`** — `server.py` called `STATE._executor.shutdown(wait=False)` early to let other teardown run in parallel, but a probe that had already cleared the `s.running` guard continued through the full `_run_once` path, including the alert-engine's `db_get_stage_state` read against `alert_profile_state`. If that query was mid-flight when `pg_close_pool()` ran, the cursor died with `InterfaceError: cursor already closed`. Added a final explicit `STATE._executor.shutdown(wait=True)` barrier immediately before `pg_close_pool()` — bounded by the probe hard-timeout cap (worst case ~90 s for an in-flight VMware first-connect; typically sub-second)
- **Alert batcher's atexit drain ran after pool close** — `monitoring/alert_batcher.shutdown()` only sets `_SHUTDOWN`; the actual drain happens on the daemon flusher's next wakeup or via the `atexit` handler, both of which fire *after* `main()`'s `pg_close_pool()`. Every queued alert then tried `_dispatch_single → db_log_event / db_has_acked_event / db_record_stage_fire`, each logging "pool is closed". Added `monitoring.alert_batcher.shutdown_sync()` — signals `_SHUTDOWN` **and** calls `_drain_all()` inline. `server.py` shutdown sequence now invokes it right after the executor barrier, before the pool close. Safe to double-call: both paths pop buckets under `_QUEUE_LOCK`, so the later atexit pass is a no-op
- **Files** — [server.py](Pingwatch/server.py) (second `STATE._executor.shutdown(wait=True)` + `shutdown_sync` call, both guarded with try/except + warning log); [monitoring/alert_batcher.py](Pingwatch/monitoring/alert_batcher.py) (`shutdown_sync()` public function; docstring on `shutdown()` clarifying it's the async last-resort)
...
- **Probe hard-timeout guard** — `core/state.py::_run_once()` previously called `result = s.probe()` directly on the worker thread. Every probe type has its own socket/timeout, but a misbehaving stack (stuck DNS resolution, TLS handshake hanging mid-read) could outlive it and pin one of the 64 executor workers. Now wraps the call in a daemon `pw-probe-{did}-{sid}` thread and `join(timeout=_cap)` where `_cap = max((s.timeout or 5) + 3, per_stype_floor)`. Per-stype floors: **vmware 90s** (SmartConnect caps at 60s internally on first session setup), **smtp 60s** (5-stage probe × timeout), **ssh 45s**, **sftp 60s**; all other probes default to **15s**. If the cap is hit the worker returns with `{"ok": False, "detail": "Probe exceeded hard timeout"}` and logs a WARN to `log_sensors` including the cap value; the orphan probe thread continues until its own internal timeout fires. Without these per-stype floors, every VMware probe after a cold restart was killed at 13 s before SmartConnect could finish — manifesting as a full-fleet DOWN storm that recovered one cycle later once the session cache populated
